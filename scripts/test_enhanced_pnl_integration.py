...
from scripts.paper_run import calculate_enhanced_pnl
...
async def test_scenario_1_all_profitable():
    ...
    assert pnl['unrealized_pnl'] == expected_unrealized, f"Expected unrealized {expected_unrealized}, got {pnl['unrealized_pnl']}"
    assert pnl['realized_pnl'] == expected_realized, f"Expected realized {expected_realized}, got {pnl['realized_pnl']}"
    assert pnl['total_pnl'] == expected_total, f"Expected total {expected_total}, got {pnl['total_pnl']}"
...
async def test_scenario_2_mixed_positions():
    ...
    for symbol, info in pnl['per_symbol'].items():
        if info['status'] == 'open':
            print(f"   {symbol:6} ({info['qty']:>5} shares): "
...
async def test_scenario_3_short_positions():
    ...
    print(f"\n📋 Per-Symbol Breakdown:")
    for symbol, info in pnl['per_symbol'].items():
        print(f"   {symbol:6} ({info['qty']:>5} shares @ ${info['avg_entry_price']:.2f}): "
