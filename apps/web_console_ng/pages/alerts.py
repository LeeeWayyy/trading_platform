                ...
                async def acknowledge_single() -> None:
                    ...
                    try:
                        await alert_service.acknowledge_alert(
                            event_select.value, note.strip(), user
                        )
...
async def _show_edit_dialog(
    ...
    with ui.dialog() as edit_dialog, ui.card().classes("p-6 w-[600px]"):
        ...
        email_edit = ui.input(
            label="Email",
            placeholder=mask_recipient(existing_channels["email"], "email")
            if "email" in existing_channels
            else "",
        ).classes("w-full")
        slack_edit = ui.input(
            label="Slack Webhook (leave blank to keep current)",
            placeholder=mask_recipient(existing_channels["slack"], "slack")
            if "slack" in existing_channels
            else "",
        ).classes("w-full")
        pagerduty_edit = ui.input(
            label="PagerDuty Routing Key (leave blank to keep current)",
            placeholder=mask_recipient(existing_channels["pagerduty"], "pagerduty")
            if "pagerduty" in existing_channels
            else "",
        ).classes("w-full")
        sms_edit = ui.input(
            label="SMS Phone (leave blank to keep current)",
            placeholder=mask_recipient(existing_channels["sms"], "sms")
            if "sms" in existing_channels
            else "",
        ).classes("w-full")
