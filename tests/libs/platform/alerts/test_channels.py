...
from libs.platform.alerts.channels import EmailChannel, SlackChannel, SMSChannel
...
class TestEmailChannel:
    ...
    def email_channel(self):
        """Create email channel with test credentials (SMTP only, no SendGrid fallback)."""
        # Mock the secret manager to avoid loading from environment
    ...
    @pytest.fixture()
    def email_channel_with_sendgrid(self):
        """Create email channel with SendGrid fallback enabled."""
        # Mock the secret manager to avoid loading from environment
        mock_secrets = MagicMock()
        mock_secrets.get_secret.return_value = None
    ...
    @pytest.mark.asyncio()
    async def test_send_smtp_connection_error_falls_back_to_sendgrid(
        self, email_channel_with_sendgrid
    ):
        ...
        with patch.object(aiosmtplib, "SMTP") as mock_smtp_class:
            ...
            with patch("httpx.AsyncClient") as mock_client_class:
                ...
                result = await email_channel_with_sendgrid.send(
                    recipient="user@example.com",
                    subject="Test",
                    body="Body",
                )
