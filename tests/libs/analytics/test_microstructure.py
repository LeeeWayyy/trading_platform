...
def _create_minute_bars(
    ...
    for i in range(n_bars):
        ...
        timestamps.append(datetime(target_date.year, target_date.month, target_date.day, hour, minute))
...
def _create_timestamps(target_date: date, n: int, start_hour: int = 9, start_minute: int = 30) -> list[datetime]:
    ...
    for i in range(n):
        ...
        timestamps.append(datetime(target_date.year, target_date.month, target_date.day, hour, minute, second))
    return timestamps
...
class TestRealizedVolatility:
    ...
    def test_rv_computation(
        ...
        mock_taq_provider.fetch_realized_volatility.side_effect = DataNotFoundError("no precomputed")
    ...
    def test_rv_uses_precomputed(
        ...
        result = analyzer.compute_realized_volatility("AAPL", date(2024, 1, 15), sampling_freq_minutes=5)
    ...
    def test_rv_includes_version_id(
        ...
        mock_taq_provider.fetch_minute_bars.return_value = bars
        mock_taq_provider.manifest_manager.load_manifest.return_value = MagicMock(checksum="test_version")
    ...
    def test_rv_with_as_of(
        ...
        result = analyzer.compute_realized_volatility("AAPL", date(2024, 1, 15), as_of=date(2024, 2, 1))
...
class TestVPIN:
    ...
    def test_vpin_partial_bucket(
        ...
        if result.num_buckets > 0:
            ...
            if not last_bucket.is_empty():
                assert last_bucket["is_partial"][0] == True
    ...
    def test_vpin_includes_version_id(
        ...
        ticks = _create_tick_data("AAPL", date(2024, 1, 15), n_ticks=100)
        mock_taq_provider.fetch_ticks.return_value = ticks
        mock_taq_provider.manifest_manager.load_manifest.return_value = MagicMock(checksum="vpin_version")
...
class TestIntradayPattern:
    ...
    def test_intraday_holidays(
        ...
        empty_bars = pl.DataFrame(schema={
            "ts": pl.Datetime,
            "symbol": pl.Utf8,
            "open": pl.Float64,
            "high": pl.Float64,
    ...
    def test_intraday_includes_version_id(
        ...
        bars = _create_minute_bars("AAPL", date(2024, 1, 15))
        mock_taq_provider.fetch_minute_bars.return_value = bars
        mock_taq_provider.manifest_manager.load_manifest.return_value = MagicMock(checksum="intraday_v1")
...
class TestSpreadDepth:
    ...
    def test_spread_retrieval(
        ...
        mock_taq_provider.manifest_manager.load_manifest.return_value = MagicMock(checksum="spread_v1")
    ...
    def test_depth_computation(
        ...
        mock_taq_provider.fetch_spread_metrics.return_value = spread_df
        mock_taq_provider.fetch_ticks.return_value = _create_tick_data("AAPL", date(2024, 1, 15))
        mock_taq_provider.manifest_manager.load_manifest.return_value = MagicMock(checksum="depth_v1")
    ...
    def test_depth_quote_only_filter(
        ...
        mock_taq_provider.fetch_ticks.return_value = ticks
        mock_taq_provider.manifest_manager.load_manifest.return_value = MagicMock(checksum="filter_v1")
    ...
    def test_depth_imbalance(
        ...
        mock_taq_provider.fetch_ticks.return_value = ticks
        mock_taq_provider.manifest_manager.load_manifest.return_value = MagicMock(checksum="imbal_v1")
    ...
    def test_depth_empty_book(
        ...
        mock_taq_provider.fetch_ticks.return_value = ticks
        mock_taq_provider.manifest_manager.load_manifest.return_value = MagicMock(checksum="empty_v1")
    ...
    def test_spread_depth_pit(
        ...
        result = analyzer.compute_spread_depth_stats("AAPL", date(2024, 1, 15), as_of=date(2024, 2, 1))
    ...
    def test_locked_markets(
        ...
        mock_taq_provider.fetch_ticks.return_value = ticks
        mock_taq_provider.manifest_manager.load_manifest.return_value = MagicMock(checksum="locked_v1")

        result = analyzer.compute_spread_depth_stats("AAPL", date(2024, 1, 15))

        assert result.has_locked_markets == True
    ...
    def test_crossed_markets(
        ...
        mock_taq_provider.fetch_ticks.return_value = ticks
        mock_taq_provider.manifest_manager.load_manifest.return_value = MagicMock(checksum="crossed_v1")

        result = analyzer.compute_spread_depth_stats("AAPL", date(2024, 1, 15))

        assert result.has_crossed_markets == True
    ...
    def test_stale_quotes_high_pct(
        ...
        timestamps = [
            datetime(2024, 1, 15, 9, 30) + timedelta(seconds=i * 120)
            for i in range(10)
        ]
        ticks = pl.DataFrame({
            "ts": timestamps,
        ...
        mock_taq_provider.fetch_ticks.return_value = ticks
        mock_taq_provider.manifest_manager.load_manifest.return_value = MagicMock(checksum="stale_v1")
    ...
    def test_spread_only_fallback(
        ...
        mock_taq_provider.manifest_manager.load_manifest.return_value = MagicMock(checksum="fallback_v1")

        result = analyzer.compute_spread_depth_stats("AAPL", date(2024, 1, 15))

        assert result.depth_is_estimated == True
...
class TestDeterminism:
    ...
    def test_depth_calculation_deterministic(
        ...
        ticks = _create_tick_data("AAPL", date(2024, 1, 15), n_ticks=100)
        mock_taq_provider.fetch_ticks.return_value = ticks
        mock_taq_provider.manifest_manager.load_manifest.return_value = MagicMock(checksum="det_depth")
