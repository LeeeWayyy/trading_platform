...
class _RedisClient:
    ...
    def mget(self, keys): return [None for _ in keys]
    def get(self, key): return None
    def smembers(self, key): return []
    def delete(self, *keys): return True
    def set(self, key, val, ttl=None): return True
    def pipeline(self): return self
    def sadd(self, key, *members): return 0
    def expire(self, *_args, **_kwargs): return True
...
class DummyDB:
    ...
    def get_order_by_client_id(self, cid): return self.orders.get(cid)
    def create_order(self, client_order_id, strategy_id, order_request, status, broker_order_id=None, error_message=None):
        od = main.OrderDetail(
    ...
    def get_positions_for_strategies(self, _strats):
        return self.positions
    def get_position_by_symbol(self, symbol): return 0
    def get_daily_pnl_history(self, start, end, strats): return self.daily_rows
    def get_data_availability_date(self): return None
    def transaction(self):
        class Tx:
            def __enter__(self_inner): return self_inner
            def __exit__(self_inner, exc_type, exc, tb): return False
        return Tx()
    def get_order_for_update(self, cid, conn): return self.orders.get(cid)
    def get_position_for_update(self, symbol, conn): return None
...
class DummyKillSwitch:
    def __init__(self): self.engaged = False
    def is_engaged(self): return self.engaged
    def engage(self, **_): self.engaged = True
    def disengage(self, **_): self.engaged = False
    def get_status(self): return {"state": "ENGAGED" if self.engaged else "ACTIVE"}


class DummyBreaker:
    def __init__(self, tripped=False): self._tripped = tripped
    def is_tripped(self): return self._tripped
    def get_trip_reason(self): return "test trip"


class DummyReservation:
    def __init__(self): self.release_calls = 0; self.confirm_calls = 0
    def reserve(self, **_):
        return SimpleNamespace(success=True, token="t", previous_position=0, new_position=1, reason=None)
    def release(self, *_args): self.release_calls += 1
    def confirm(self, *_args): self.confirm_calls += 1
...
def app_client(monkeypatch):
    ...
    fake_db.daily_rows = [
        {"trade_date": date(2024, 1, 1), "daily_realized_pl": Decimal("1"), "closing_trade_count": 1}
    ]
...
def test_kill_switch_endpoints(app_client):
    engage = app_client.post("/api/v1/kill-switch/engage", json={"reason": "test", "operator": "op"})
