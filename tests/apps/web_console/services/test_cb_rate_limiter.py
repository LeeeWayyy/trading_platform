...
class TestCBRateLimiter:
    ...
    def test_first_reset_allowed_with_set_nx(
        self, rate_limiter: CBRateLimiter, mock_redis: MagicMock
    ) -> None:
        """First reset should be allowed (SET NX succeeds)."""
        # SET NX returns True when key was set (not exists)
        mock_redis.set.return_value = True

        result = rate_limiter.check_global(limit=1, window=60)

        assert result is True
        mock_redis.set.assert_called_once_with(rate_limiter.key, "1", nx=True, ex=60)
        # INCR should not be called for limit=1
        mock_redis.incr.assert_not_called()

    def test_second_reset_within_minute_blocked_with_set_nx(
        self, rate_limiter: CBRateLimiter, mock_redis: MagicMock
    ) -> None:
    ...
    def test_expire_only_called_on_first_increment(
        self, rate_limiter: CBRateLimiter, mock_redis: MagicMock
    ) -> None:
        """EXPIRE should only be called when count is 1 (limit > 1 path)."""
    ...
    def test_first_incr_sets_expire(
        self, rate_limiter: CBRateLimiter, mock_redis: MagicMock
    ) -> None:
        """First INCR (returning 1) should set expire (limit > 1 path)."""
        mock_redis.incr.return_value = 1

        rate_limiter.check_global(limit=3, window=120)

        mock_redis.incr.assert_called_once_with(rate_limiter.key)
        mock_redis.expire.assert_called_once_with(rate_limiter.key, 120)
...
class TestCBRateLimiterConcurrency:
    ...
    def test_concurrent_resets_atomic_check(self, mock_redis: MagicMock) -> None:
        """Only one concurrent reset should pass due to atomic SET NX.
        ...
        # Simulate two concurrent SET NX calls
        # First call succeeds (returns True), second fails (returns None)
