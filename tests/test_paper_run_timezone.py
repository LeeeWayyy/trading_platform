...
class TestConsoleOutputTimezone:
    ...
    def test_console_output_timestamp_format(self, capsys):
        ...
        assert (
            "2025-01-17T09:00:00+00:00" in output
    ...
    def test_console_output_timestamp_includes_timezone(self, capsys):
        ...
        assert "+00:00" in output, "Timestamp must include timezone offset (+00:00 for UTC)"
    ...
    def test_console_output_uses_utc_timezone(self, capsys):
        ...
        captured = capsys.readouterr()
        output = captured.out
        assert "2025-01-17T14:30:00+00:00" in output, "Timestamp should be in UTC timezone"
...
class TestJSONExportTimezone:
    ...
    async def test_json_export_timestamp_format(self, tmp_path):
        ...
        result = {"run_id": "test-run-123", "status": "completed", "mappings": []}
    ...
    async def test_json_export_includes_timezone_field(self, tmp_path):
        ...
        result = {"run_id": "test-run-456", "status": "completed", "mappings": []}
        pnl_metrics = create_simple_pnl_metrics(Decimal("30000"))

        # Act
        await save_results(config, result, pnl_metrics, datetime.now(UTC))
    ...
    async def test_json_timestamp_is_utc_not_local(self, tmp_path):
        ...
        result = {"run_id": "test-run-789", "status": "completed", "mappings": []}
    ...
    async def test_json_timestamp_ends_with_utc_offset(self, tmp_path):
        ...
        result = {"run_id": "test-run-101", "status": "completed", "mappings": []}
    ...
    async def test_json_timestamp_parseable_with_timezone(self, tmp_path):
        ...
        result = {"run_id": "test-run-202", "status": "completed", "mappings": []}
        ...
        assert parsed_dt.tzinfo is not None, "Parsed timestamp should be timezone-aware"

        # Verify it's UTC
        assert parsed_dt.tzinfo == UTC, "Parsed timestamp should be in UTC timezone"
...
class TestTimezoneConsistency:
    ...
    async def test_console_and_json_use_same_timestamp(self, tmp_path, capsys):
        ...
        result = {"run_id": "test-run-303", "status": "completed", "mappings": []}
        pnl_metrics = create_simple_pnl_metrics(Decimal("35000"))

        # Generate timestamp once (simulating what main() does)
        run_timestamp = datetime(2025, 1, 17, 12, 0, 0, tzinfo=UTC)
        ...
        captured = capsys.readouterr()
        assert (
            "2025-01-17T12:00:00+00:00" in captured.out
        ...
        assert (
            data["timestamp"] == "2025-01-17T12:00:00+00:00"
...
class TestTimezoneRegression:
    ...
    async def test_no_naive_datetime_in_json(self, tmp_path):
        ...
        result = {"run_id": "test-run-404", "status": "completed", "mappings": []}
        ...
        assert (
            "+" in timestamp_str or "Z" in timestamp_str
...
class TestEnhancedPnLTimezone:
    ...
    async def test_json_with_enhanced_pnl_includes_timezone(self, tmp_path):
        ...
        result = {"run_id": "test-run-505", "status": "completed", "mappings": []}
