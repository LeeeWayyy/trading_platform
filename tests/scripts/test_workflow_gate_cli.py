...
from workflow_gate import main, PROJECT_ROOT
...
class TestWorkflowGateCLI:
    ...
    def test_check_context_command(self):
        ...
        with patch('workflow_gate.WorkflowGate'):
            with patch('workflow_gate.DelegationRules') as MockRules:
                ...
                mock_rules_instance.should_delegate_context.return_value = (False, "OK - 50.0%", None)
    ...
    def test_record_review_command(self):
        ...
        with patch('workflow_gate.WorkflowGate') as MockGate:
            ...
            with patch.object(sys, 'argv', ['workflow_gate.py', 'record-review', 'cont-123', 'APPROVED']):
                exit_code = main()

            assert exit_code == 0
            mock_gate_instance.record_review.assert_called_once_with('cont-123', 'APPROVED')
    ...
    def test_record_delegation_command(self):
        ...
        with patch('workflow_gate.WorkflowGate'):
            with patch('workflow_gate.DelegationRules') as MockRules:
                ...
                with patch.object(sys, 'argv', ['workflow_gate.py', 'record-delegation', 'Test delegation task']):
                    exit_code = main()

                assert exit_code == 0
                mock_rules_instance.record_delegation.assert_called_once_with('Test delegation task')
        ...
        with patch('workflow_gate.WorkflowGate'):
            with patch('workflow_gate.SmartTestRunner') as MockRunner:
                with patch('workflow_gate.subprocess.run') as mock_subprocess:
                    ...
                    mock_runner_instance.get_test_command.return_value = ["poetry", "run", "pytest", "tests/"]
