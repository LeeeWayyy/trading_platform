# OpenClaw Apex Labs — Setup Plan

**Companion document:** [JARVIS_APEX_LABS_BRIEFING.md](./JARVIS_APEX_LABS_BRIEFING.md) — feed this to your Jarvis agent.
...
This document covers the technical setup for Apex Labs. For Jarvis integration, see the companion briefing document.
...
1. **Jarvis (your existing personal agent)** — Wei's personal AI assistant. Jarvis lives OUTSIDE the company org chart and is NOT defined in this plan. For Jarvis's role and commands, see the [companion briefing](./JARVIS_APEX_LABS_BRIEFING.md).
...
3. **The Roles** — The Apex Labs team. Split into two categories:
   ...
   - **ACP-backed Claude Code agents** (Lead Quant, Data Engineer, Core Trading Engineer, Platform Services Engineer, Frontend Engineer, DevOps/SRE): Use Claude Code as an external runtime via the Agent Client Protocol (ACP). They automatically get Claude Code's `CLAUDE.md` context from their cwd (assumption -- verify with a smoke test during setup).
...
> **Note:** The `--agent jarvis` flag explicitly binds these cron jobs to Jarvis. The cron job message tells Jarvis to spawn Atlas.
...
- Sub-agents receive ONLY `AGENTS.md` + `TOOLS.md`. Excluded: `SOUL.md`, `IDENTITY.md`, `USER.md`, `HEARTBEAT.md`, `BOOTSTRAP.md`.
...
```json5
{
  ...
  agents: {
    ...
    list: [
      ...
      // See the companion briefing document (JARVIS_APEX_LABS_BRIEFING.md).
      // ══════════════════════════════════════════════
      ...
      // (assumption — verify with a smoke test during setup).
      ...
      // ROLE 3: Lead Quant (split into 2 agents by cwd)
      {
        ...
        tools: {
          profile: "coding",
          deny: ["sessions_spawn", "sessions_list", "sessions_history", "sessions_send"],
        },
      },
      {
  ...
  //              Does NOT include `search`, `fetch`, `browser`, or `agent`.
  //   full:      Unrestricted — all tools available.
  //
  // NOTE: Explicit `allow` entries matter. The `minimal` profile only includes
  // `session_status` — if you need `sessions_spawn`, `read`, `write`, `edit`, etc.,
...
**ACP agents get Claude Code's built-in tools via the ACP runtime.** Additionally, add a minimal `TOOLS.md` to each ACP agent's workspace listing the key tools available (see Step 3.6 for the template). They also get an `AGENTS.md` in their workspace for role/scope instructions.

**When spawned as sub-agents** (via `sessions_spawn`), agents receive ONLY `AGENTS.md` + `TOOLS.md` from their workspace. Excluded: `SOUL.md`, `IDENTITY.md`, `USER.md`, `HEARTBEAT.md`, `BOOTSTRAP.md`. Since Atlas and role agents are always spawned as sub-agents in this architecture, all their persona information goes in `AGENTS.md`. Note: if an agent is invoked directly (e.g., `openclaw agent --agent atlas`), it loads all workspace bootstrap files like a top-level agent.
...
### Step 3.1: Atlas — Apex Labs Orchestrator
...
**IMPORTANT: `sessions_spawn` is asynchronous** — it returns immediately. You cannot wait for a sub-agent's result in the same turn. Design your orchestration as a state machine: spawn the work, end your turn, and handle the result when re-triggered by Jarvis/webhook/cron.
...
### Step 3.2: Lead Trader Workspace
...
### Step 3.4: QA Engineer Workspace
...
**Note:** Lead Quant (ACP agent) uses the shared ACP TOOLS.md template. See Step 3.6.
...
### Step 3.6: ACP Agents — Shared TOOLS.md Template
...
2. **CLAUDE.md context** — Project-wide rules and coding standards loaded from their cwd (assumption -- verify with a smoke test during setup)
...
- ACP agents: `AGENTS.md` + `TOOLS.md` (from the ACP template in Step 3.6) go in the shared workspace directory
