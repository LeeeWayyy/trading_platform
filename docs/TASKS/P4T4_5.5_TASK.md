...
**Plan Review:** ✅ Approved with Comments (Gemini + Codex, 2025-12-14)
...
| C | Use IC time series as proxy | IC ≠ returns (different statistical interpretation) |
...
| **P-Value** | `sum(simulated >= observed) / n_simulations` | One-sided; tests null hypothesis of no skill |
...
1. ✅ Option A (add daily_portfolio_returns to BacktestResult) is architecturally sound
2. ⚠️ Backward compatibility in result_storage.py is CRITICAL
3. ➕ Add `mean_ic_ci` - cheap to compute from existing daily_ic series
4. ➕ Add `hit_rate_ci` as Portfolio Hit Rate (% of days with return >= 0)
5. ✅ Date alignment: label returns with date T (decision date)

### Codex (planner role) - APPROVED WITH COMMENTS
1. ✅ Pin schema for daily_portfolio_returns: `{date: pl.Date, return: pl.Float64}`
2. ⚠️ Define metric formulas clearly (Sharpe annualization, drawdown calc)
3. ⚠️ Clarify bootstrap vs shuffle (with vs without replacement)
4. ➕ Document p-value interpretation (one-sided)
5. ➕ Consider extensibility for future metrics (Sortino, skew)
