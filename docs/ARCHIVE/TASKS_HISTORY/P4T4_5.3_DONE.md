...
**Parent Task:** [P4T4_DONE.md](./P4T4_DONE.md)
**Task ID:** P4T4-T5.3
...
from libs.trading.backtest.job_queue import BacktestJobQueue, BacktestJobConfig, JobPriority
from libs.trading.backtest.result_storage import BacktestResultStorage
from libs.trading.alpha.alpha_library import CANONICAL_ALPHAS  # Use existing alpha registry
...
|---|-----------|--------|-------|
| C1 | Auth Stub & Governance Tests | ⏳ In Progress | `backtest_auth.py`, `test_auth_governance.py` |
| C2 | Backtest Form Component | 📋 Pending | `backtest_form.py` |
| C3 | Visualization Components | 📋 Pending | `equity_curve_chart.py`, `ic_timeseries_chart.py`, `drawdown_chart.py`, `backtest_results.py` |
| C4 | Main Backtest Page (incl. comparison & export) | 📋 Pending | `pages/backtest.py`, `app.py`, `config.py` |
| C5 | Test Suite | 📋 Pending | `test_backtest_page.py`, `test_backtest_job_status.py`, `test_auth_enforcement.py` |
| C6 | Documentation | 📋 Pending | `backtest-web-ui.md`, `ADR-0025-*.md` |
...
- [ ] T6.1 Auth: Pending → Using dev-mode stub
...
- Side-by-side metrics table showing ≥5 metrics: mean_ic, icir, hit_rate, coverage, average_turnover
...
def render_export_buttons(result: BacktestResult, user_info: dict[str, Any]) -> None:
    ...
    with col1:
        st.download_button("📊 Signals CSV", signals_csv, "signals.csv", "text/csv")
    # ... etc
```
...
- Polling: Progressive backoff (2s→5s→10s→30s)
...
if job["status"] == "failed":
    ...
    st.warning(f"⚠️ Cancelled by user")
```
