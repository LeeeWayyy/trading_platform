...
- **Threshold:** Job marked failed if heartbeat older than `max(job_timeout, 300)` seconds (300s floor for quick detection)
...
class BacktestJobQueue:
    ...
    def _create_db_job(
        ...
        *,
        ...
        # Schema reference: See P4T4_DONE.md "Result Storage Schema" for authoritative DDL.
    ...
    def enqueue(
        ...
        *,
        ...
        """
        from libs.trading.backtest.models import BacktestJob
    ...
    def watchdog_fail_lost_jobs(self) -> int:
        ...
        for job in running_jobs:
            threshold = now_ts - max(int(job["job_timeout"]), 300)  # 300s floor
...
from libs.trading.alpha.research_platform import PITBacktester, BacktestResult
from libs.trading.backtest.job_queue import BacktestJobConfig, BacktestJobQueue
...
def run_backtest(config: dict[str, Any], created_by: str) -> dict[str, Any]:
    ...
    with db_pool.connection() as conn:
        ...
        try:
            ...
            from libs.trading.alpha.registry import get_alpha_by_name
...
def _save_parquet_artifacts(job_id: str, result: BacktestResult) -> Path:
    ...
    Schema Contract: See P4T4_DONE.md "Parquet Schema Contract (Authoritative)" section.
