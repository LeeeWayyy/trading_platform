# T1.3 CRSP Local Provider - Implementation Plan

**Component:** T1.3-CRSP-Local-Provider
**Status:** Planning (v1.1 - Revised per Gemini + Codex review)
**Created:** 2025-12-04
**Revised:** 2025-12-04

---

## Overview

The CRSP Local Provider is a **read-only** provider that uses DuckDB to query CRSP daily data stored in Parquet files created by the T1.2 Sync Manager.

### Key Features
1. **Point-in-time universe construction** - Avoid look-ahead bias using first/last trade dates
2. **Survivorship-bias-free data access** - Include delisted stocks by default
3. **Ticker-to-PERMNO historical mapping** - Track ticker changes with ambiguity handling
4. **Manifest-aware partition pruning** - Use manifest entries for consistency
5. **Reader snapshot consistency** - Gate queries on manifest version

### Dependencies
- T1.1 Data Quality Framework - **COMPLETE** (a4e0e8f)
- T1.2 WRDS Sync Manager - **COMPLETE** (98ee32b)

---

## Review Feedback Addressed (v1.1)

| Reviewer | Severity | Issue | Resolution |
|----------|----------|-------|------------|
| Gemini | HIGH | Storage path mismatch | Fixed: `data/wrds/crsp/daily/` per P4T1_TASK.md |
| Gemini | HIGH | Missing reader consistency | Added: Manifest version pinning in all queries |
| Gemini | MEDIUM | Negative price handling | Added: `adjust_prices` flag with clear docs |
| Gemini | MEDIUM | Ticker mapping ambiguity | Changed: Raise `AmbiguousTickerError` instead of None |
| Codex | HIGH | Missing IPO/delist source | Added: Derive from first/last trade dates + metadata cache |
| Codex | HIGH | No manifest awareness | Added: ManifestManager integration and version pinning |
| Codex | MEDIUM | Query injection risk | Added: Parameterized DuckDB queries |
| Codex | MEDIUM | Path traversal risk | Added: Path validation against data root |
| Codex | MEDIUM | Manifest-driven pruning | Added: Use manifest file_paths, not filesystem |
| Codex | MEDIUM | Column validation | Added: Validate against CRSP_SCHEMA constant |
| Codex | LOW | Edge case tests | Added: Future date, empty range, missing partition tests |
| Codex | HIGH | adjust_prices ineffective with columns=None | Fixed: Apply ABS(prc) via explicit column list when adjust_prices=True |
| Codex | HIGH | Non-deterministic column order with set | Fixed: Use ordered CRSP_COLUMNS tuple instead of VALID_COLUMNS set |

**Non-blocking observations addressed:**
- `as_of_date` intentionally only filters IPO (first_date), not delisting - this is correct for survivorship-bias-free analysis
- Metadata cache invalidation documented in `invalidate_cache()` docstring

---

## Files to Create

### 1. libs/data_providers/crsp_local_provider.py

Main provider implementation:

```python
"""CRSP Local Data Provider.

Read-only provider for querying CRSP daily data stored in Parquet files.
Implements manifest-aware snapshot consistency and point-in-time filtering.
"""

from __future__ import annotations

import logging
from datetime import date
from pathlib import Path
from typing import TYPE_CHECKING

import duckdb
import polars as pl

from libs.data.data_quality.exceptions import DataNotFoundError
from libs.data.data_quality.manifest import ManifestManager, SyncManifest

if TYPE_CHECKING:
    pass

logger = logging.getLogger(__name__)


class AmbiguousTickerError(Exception):
    """Raised when a ticker maps to multiple PERMNOs on the same date."""

    def __init__(self, ticker: str, as_of_date: date, permnos: list[int]) -> None:
        self.ticker = ticker
        self.as_of_date = as_of_date
        self.permnos = permnos
        super().__init__(
            f"Ticker '{ticker}' is ambiguous on {as_of_date}: "
            f"maps to PERMNOs {permnos}"
        )


class ManifestVersionChangedError(Exception):
    """Raised when manifest version changes during query execution."""

    pass


# Schema definition for validation (ordered for deterministic column selection)
CRSP_COLUMNS = ("date", "permno", "cusip", "ticker", "ret", "prc", "vol", "shrout")

CRSP_SCHEMA = {
    "date": pl.Date,
    "permno": pl.Int64,
    "cusip": pl.Utf8,
    "ticker": pl.Utf8,
    "ret": pl.Float64,
    "prc": pl.Float64,  # Note: Negative = bid/ask average
    "vol": pl.Float64,
    "shrout": pl.Float64,
}

VALID_COLUMNS = set(CRSP_COLUMNS)  # For O(1) validation lookups


class CRSPLocalProvider:
    """Read-only provider for CRSP daily data.

    Uses DuckDB to query Parquet files with manifest-aware partition pruning.
    Implements reader snapshot consistency by pinning manifest version.

    Storage Layout (per P4T1_TASK.md):
        data/wrds/crsp/daily/
        ├── 2020.parquet
        ├── 2021.parquet
        └── 2024.parquet

    Each parquet file contains:
        - date: Date of trading day
        - permno: CRSP permanent identifier
        - cusip: CUSIP identifier
        - ticker: Stock ticker symbol
        - ret: Holding period return
        - prc: Closing price (NEGATIVE = bid/ask average, use adjust_prices=True)
        - vol: Trading volume
        - shrout: Shares outstanding

    Thread Safety:
        This provider is thread-safe for read operations.
        Each query pins the manifest version at start.

    Example:
        provider = CRSPLocalProvider(
            storage_path=Path("data/wrds/crsp/daily"),
            manifest_manager=manifest_mgr,
        )

        # Get prices with manifest consistency
        df = provider.get_daily_prices(
            start_date=date(2023, 1, 1),
            end_date=date(2023, 12, 31),
        )
    """

    DATASET_NAME = "crsp_daily"
    DATA_ROOT = Path("data")  # Permitted root for security validation

    def __init__(
        self,
        storage_path: Path,
        manifest_manager: ManifestManager,
        data_root: Path | None = None,
    ) -> None:
        """Initialize CRSP provider.

        Args:
            storage_path: Path to CRSP data directory (e.g., data/wrds/crsp/daily).
            manifest_manager: Manager for manifest operations (required for consistency).
            data_root: Root directory for path validation (default: data/).

        Raises:
            ValueError: If storage_path is outside data_root (security).
        """
        self.storage_path = Path(storage_path).resolve()
        self.manifest_manager = manifest_manager
        self.data_root = (data_root or self.DATA_ROOT).resolve()

        # Security: Validate storage_path is within data_root
        if not self.storage_path.is_relative_to(self.data_root):
            raise ValueError(
                f"storage_path '{storage_path}' must be within data_root '{self.data_root}'"
            )

        self._conn: duckdb.DuckDBPyConnection | None = None
        self._security_metadata: pl.DataFrame | None = None

    def get_daily_prices(
        self,
        start_date: date,
        end_date: date,
        symbols: list[str] | None = None,
        permnos: list[int] | None = None,
        as_of_date: date | None = None,
        columns: list[str] | None = None,
        adjust_prices: bool = False,
    ) -> pl.DataFrame:
        """Get daily price data for securities in date range.

        Implements Reader Snapshot Consistency:
        1. Read manifest version at query start
        2. Execute query
        3. Verify manifest version unchanged (retry if changed)

        Args:
            start_date: Start of date range (inclusive).
            end_date: End of date range (inclusive).
            symbols: Filter by ticker symbols (None = all).
            permnos: Filter by PERMNOs (None = all).
            as_of_date: Point-in-time filter - exclude securities IPO'd after this date.
            columns: Columns to return (None = all). Validated against schema.
            adjust_prices: If True, return abs(prc) instead of raw (negative = bid/ask).

        Returns:
            DataFrame with requested columns.

        Raises:
            ValueError: If invalid columns requested.
            DataNotFoundError: If no manifest found (run sync first).
            ManifestVersionChangedError: If manifest changes during query (retry).
        """
        # Validate columns
        if columns is not None:
            invalid = set(columns) - VALID_COLUMNS
            if invalid:
                raise ValueError(
                    f"Invalid columns: {invalid}. Valid: {VALID_COLUMNS}"
                )

        # Pin manifest version for snapshot consistency
        manifest = self._get_manifest()
        pinned_version = manifest.manifest_version

        # Get partition paths from manifest (not filesystem)
        partition_paths = self._get_partition_paths_from_manifest(
            manifest, start_date, end_date
        )

        if not partition_paths:
            return self._empty_result(columns)

        # Build and execute query with parameterization
        result = self._execute_query(
            partition_paths=partition_paths,
            start_date=start_date,
            end_date=end_date,
            symbols=symbols,
            permnos=permnos,
            as_of_date=as_of_date,
            columns=columns,
            adjust_prices=adjust_prices,
        )

        # Verify manifest version unchanged (snapshot consistency)
        current_manifest = self._get_manifest()
        if current_manifest.manifest_version != pinned_version:
            raise ManifestVersionChangedError(
                f"Manifest version changed from {pinned_version} to "
                f"{current_manifest.manifest_version} during query"
            )

        return result

    def get_universe(
        self,
        as_of_date: date,
        include_delisted: bool = True,
    ) -> pl.DataFrame:
        """Get universe of securities as of given date.

        Point-in-time logic:
        - IPO date = first trade date in data (security's min date)
        - Delist date = last trade date in data (security's max date)
        - Security included if: first_date <= as_of_date
        - If include_delisted=False: also require last_date >= as_of_date

        Args:
            as_of_date: Reference date for universe construction.
            include_delisted: If True (default), include stocks delisted before as_of_date.
                            If False, only include stocks actively trading on as_of_date.

        Returns:
            DataFrame with: permno, ticker, cusip, first_date, last_date
        """
        metadata = self._get_security_metadata()

        # Filter: security must have IPO'd by as_of_date
        result = metadata.filter(pl.col("first_date") <= as_of_date)

        if not include_delisted:
            # Also require: security still trading on as_of_date
            result = result.filter(pl.col("last_date") >= as_of_date)

        return result

    def ticker_to_permno(
        self,
        ticker: str,
        as_of_date: date,
    ) -> int:
        """Map ticker symbol to PERMNO at given date.

        Args:
            ticker: Ticker symbol to look up.
            as_of_date: Date for the lookup.

        Returns:
            PERMNO that the ticker referred to on as_of_date.

        Raises:
            DataNotFoundError: If ticker not found on as_of_date.
            AmbiguousTickerError: If ticker maps to multiple PERMNOs on as_of_date.
        """
        metadata = self._get_security_metadata()

        # Find securities with this ticker that were active on as_of_date
        matches = metadata.filter(
            (pl.col("ticker") == ticker.upper())
            & (pl.col("first_date") <= as_of_date)
            & (pl.col("last_date") >= as_of_date)
        )

        if matches.is_empty():
            raise DataNotFoundError(
                f"Ticker '{ticker}' not found or not active on {as_of_date}"
            )

        permnos = matches["permno"].to_list()
        if len(permnos) > 1:
            raise AmbiguousTickerError(ticker, as_of_date, permnos)

        return permnos[0]

    def permno_to_ticker(
        self,
        permno: int,
        as_of_date: date,
    ) -> str:
        """Map PERMNO to ticker symbol at given date.

        Args:
            permno: PERMNO to look up.
            as_of_date: Date for the lookup.

        Returns:
            Ticker symbol that was valid for the PERMNO on as_of_date.

        Raises:
            DataNotFoundError: If PERMNO not found or not trading on as_of_date.
        """
        # Query the actual data to get ticker on specific date
        # (ticker can change over time for same PERMNO)
        manifest = self._get_manifest()
        paths = self._get_partition_paths_from_manifest(
            manifest, as_of_date, as_of_date
        )

        if not paths:
            raise DataNotFoundError(
                f"No data available for {as_of_date}"
            )

        conn = self._ensure_connection()

        # Use parameterized query
        query = """
            SELECT DISTINCT ticker
            FROM read_parquet($paths)
            WHERE permno = $permno AND date = $as_of_date
        """
        result = conn.execute(
            query,
            {"paths": [str(p) for p in paths], "permno": permno, "as_of_date": as_of_date}
        ).pl()

        if result.is_empty():
            raise DataNotFoundError(
                f"PERMNO {permno} not found or not trading on {as_of_date}"
            )

        return result["ticker"][0]

    def get_security_timeline(
        self,
        permno: int,
    ) -> pl.DataFrame:
        """Get complete trading history for a security.

        Useful for tracking ticker changes over time.

        Args:
            permno: PERMNO to look up.

        Returns:
            DataFrame with: date, ticker, prc, ret, vol (ordered by date).

        Raises:
            DataNotFoundError: If PERMNO has no trading history.
        """
        manifest = self._get_manifest()
        paths = [Path(p) for p in manifest.file_paths]

        if not paths:
            raise DataNotFoundError("No CRSP data available")

        conn = self._ensure_connection()

        query = """
            SELECT date, ticker, prc, ret, vol
            FROM read_parquet($paths)
            WHERE permno = $permno
            ORDER BY date
        """
        result = conn.execute(
            query,
            {"paths": [str(p) for p in paths], "permno": permno}
        ).pl()

        if result.is_empty():
            raise DataNotFoundError(f"PERMNO {permno} has no trading history")

        return result

    def _get_manifest(self) -> SyncManifest:
        """Load manifest for CRSP daily data.

        Raises:
            DataNotFoundError: If no manifest (run sync first).
        """
        manifest = self.manifest_manager.load_manifest(self.DATASET_NAME)
        if manifest is None:
            raise DataNotFoundError(
                f"No manifest found for '{self.DATASET_NAME}'. "
                f"Run full_sync first."
            )
        return manifest

    def _get_partition_paths_from_manifest(
        self,
        manifest: SyncManifest,
        start_date: date,
        end_date: date,
    ) -> list[Path]:
        """Get partition paths from manifest for date range.

        Uses manifest file_paths (not filesystem) for consistency.
        Only returns paths for years overlapping with date range.

        Args:
            manifest: Current manifest.
            start_date: Start of range.
            end_date: End of range.

        Returns:
            List of parquet file paths needed for query.
        """
        needed_years = set(range(start_date.year, end_date.year + 1))
        paths = []

        for path_str in manifest.file_paths:
            path = Path(path_str)
            # Extract year from filename (e.g., "2024.parquet" -> 2024)
            try:
                year = int(path.stem)
                if year in needed_years:
                    # Verify path is within data_root (security)
                    resolved = path.resolve()
                    if resolved.is_relative_to(self.data_root):
                        paths.append(path)
                    else:
                        logger.warning(
                            "Skipping path outside data_root: %s",
                            path,
                        )
            except ValueError:
                # Filename doesn't match year pattern
                continue

        return paths

    def _execute_query(
        self,
        partition_paths: list[Path],
        start_date: date,
        end_date: date,
        symbols: list[str] | None,
        permnos: list[int] | None,
        as_of_date: date | None,
        columns: list[str] | None,
        adjust_prices: bool,
    ) -> pl.DataFrame:
        """Execute parameterized DuckDB query.

        All filtering uses parameterized queries to prevent SQL injection.
        """
        conn = self._ensure_connection()

        # Build column list
        # When adjust_prices=True, we must use explicit columns (not *)
        # to apply ABS(prc) transformation
        if columns is None:
            if adjust_prices:
                # Use ordered CRSP_COLUMNS tuple for deterministic column order
                cols = [c if c != "prc" else "ABS(prc) AS prc" for c in CRSP_COLUMNS]
                col_expr = ", ".join(cols)
            else:
                col_expr = "*"
        else:
            # Add prc handling for adjust_prices
            if adjust_prices and "prc" in columns:
                cols = [c if c != "prc" else "ABS(prc) AS prc" for c in columns]
                col_expr = ", ".join(cols)
            else:
                col_expr = ", ".join(columns)

        # Build WHERE clause with parameters
        params: dict[str, object] = {
            "paths": [str(p) for p in partition_paths],
            "start_date": start_date,
            "end_date": end_date,
        }

        where_clauses = ["date >= $start_date", "date <= $end_date"]

        if symbols is not None:
            # Normalize to uppercase
            params["symbols"] = [s.upper() for s in symbols]
            where_clauses.append("ticker = ANY($symbols)")

        if permnos is not None:
            params["permnos"] = permnos
            where_clauses.append("permno = ANY($permnos)")

        if as_of_date is not None:
            # Point-in-time: exclude securities that IPO'd after as_of_date
            # This requires knowing each security's first trade date
            metadata = self._get_security_metadata()
            valid_permnos = metadata.filter(
                pl.col("first_date") <= as_of_date
            )["permno"].to_list()
            params["valid_permnos"] = valid_permnos
            where_clauses.append("permno = ANY($valid_permnos)")

        where_sql = " AND ".join(where_clauses)

        query = f"""
            SELECT {col_expr}
            FROM read_parquet($paths)
            WHERE {where_sql}
            ORDER BY date, permno
        """

        return conn.execute(query, params).pl()

    def _get_security_metadata(self) -> pl.DataFrame:
        """Get or compute security metadata (first/last trade dates).

        Caches the result for efficiency. Metadata is derived from
        the first and last trade dates in the actual data.

        Returns:
            DataFrame with: permno, ticker, cusip, first_date, last_date
        """
        if self._security_metadata is not None:
            return self._security_metadata

        manifest = self._get_manifest()
        paths = [Path(p) for p in manifest.file_paths]

        if not paths:
            self._security_metadata = pl.DataFrame(schema={
                "permno": pl.Int64,
                "ticker": pl.Utf8,
                "cusip": pl.Utf8,
                "first_date": pl.Date,
                "last_date": pl.Date,
            })
            return self._security_metadata

        conn = self._ensure_connection()

        # Compute first/last dates per PERMNO, get latest ticker/cusip
        query = """
            SELECT
                permno,
                LAST(ticker ORDER BY date) AS ticker,
                LAST(cusip ORDER BY date) AS cusip,
                MIN(date) AS first_date,
                MAX(date) AS last_date
            FROM read_parquet($paths)
            GROUP BY permno
        """
        self._security_metadata = conn.execute(
            query,
            {"paths": [str(p) for p in paths]}
        ).pl()

        return self._security_metadata

    def _ensure_connection(self) -> duckdb.DuckDBPyConnection:
        """Get or create read-only DuckDB connection.

        Uses PRAGMA disable_object_cache to ensure fresh data
        after syncs (per P4T1 DuckDB Operational Safety).
        """
        if self._conn is None:
            self._conn = duckdb.connect(":memory:", read_only=False)
            # Disable object cache for long-lived sessions
            self._conn.execute("PRAGMA disable_object_cache")
            # Set reasonable memory limit
            self._conn.execute("PRAGMA memory_limit='2GB'")
            # Limit threads for reader
            self._conn.execute("PRAGMA threads=4")

        return self._conn

    def _empty_result(self, columns: list[str] | None) -> pl.DataFrame:
        """Return empty DataFrame with correct schema."""
        if columns is None:
            schema = CRSP_SCHEMA
        else:
            schema = {c: CRSP_SCHEMA[c] for c in columns}
        return pl.DataFrame(schema=schema)

    def invalidate_cache(self) -> None:
        """Invalidate cached security metadata.

        Call this after a sync completes to refresh metadata.
        """
        self._security_metadata = None

    def close(self) -> None:
        """Close DuckDB connection."""
        if self._conn is not None:
            self._conn.close()
            self._conn = None

    def __enter__(self) -> CRSPLocalProvider:
        return self

    def __exit__(self, exc_type, exc_val, exc_tb) -> None:
        self.close()
```

### 2. tests/libs/data_providers/test_crsp_local_provider.py

Comprehensive test suite:

```python
class TestCRSPLocalProvider:
    """Tests for CRSP local data provider."""

    # === Schema Validation ===
    def test_query_returns_correct_schema(self):
        """Verify returned DataFrame has expected columns and types."""

    def test_invalid_columns_raises_error(self):
        """Requesting invalid columns raises ValueError."""

    # === Point-in-Time Tests ===
    def test_point_in_time_excludes_future_ipos(self):
        """Securities that IPO after as_of_date are excluded."""

    def test_ipo_handling_appears_on_correct_date(self):
        """Stock appears in universe on its first trading date (not before)."""

    # === Survivorship Bias Tests ===
    def test_delisted_stocks_included_by_default(self):
        """Delisted stocks are included for survivorship-bias-free analysis."""

    def test_delisting_handling_disappears_on_correct_date(self):
        """Stock disappears from active universe on delisting date."""

    def test_get_universe_exclude_delisted(self):
        """Can optionally exclude delisted stocks."""

    # === Partition Pruning Tests ===
    def test_partition_pruning_uses_manifest_not_filesystem(self):
        """Pruning uses manifest file_paths, not filesystem scan."""

    def test_partition_pruning_only_reads_needed_years(self):
        """Query for 2022 data only reads 2022.parquet."""

    def test_partition_pruning_spans_years(self):
        """Query spanning Dec 2021 - Jan 2022 reads both files."""

    def test_missing_partition_in_manifest_skipped(self):
        """Partitions in manifest but missing on disk raise error."""

    # === Ticker/PERMNO Mapping Tests ===
    def test_ticker_to_permno_mapping(self):
        """Map ticker to PERMNO at specific date."""

    def test_ticker_to_permno_not_found_raises(self):
        """Unknown ticker raises DataNotFoundError."""

    def test_ticker_to_permno_ambiguous_raises(self):
        """Ambiguous ticker raises AmbiguousTickerError with details."""

    def test_permno_to_ticker_mapping(self):
        """Map PERMNO to ticker at specific date."""

    def test_ticker_changes_historical_lookup(self):
        """Handle ticker changes over time (e.g., GOOG → GOOGL)."""

    # === Price Handling Tests ===
    def test_negative_prices_returned_raw_by_default(self):
        """Negative prices (bid/ask average) returned as-is by default."""

    def test_adjust_prices_returns_absolute_values(self):
        """adjust_prices=True returns abs(prc)."""

    # === Data Quality Tests ===
    def test_holiday_gaps_not_flagged(self):
        """Market holidays don't create spurious gaps."""

    # === Edge Case Tests ===
    def test_returns_empty_for_no_data(self):
        """Returns empty DataFrame when no data matches query."""

    def test_future_date_returns_empty(self):
        """Query for future dates returns empty DataFrame."""

    def test_date_range_no_partitions(self):
        """Query for years without partitions returns empty."""

    # === Manifest Consistency Tests ===
    def test_manifest_version_pinning(self):
        """Query pins manifest version at start."""

    def test_manifest_version_change_raises(self):
        """ManifestVersionChangedError if manifest changes during query."""

    def test_no_manifest_raises_data_not_found(self):
        """DataNotFoundError if no manifest exists."""

    # === Security Tests ===
    def test_path_outside_data_root_rejected(self):
        """Storage path outside data_root raises ValueError."""

    def test_manifest_path_traversal_blocked(self):
        """Paths in manifest outside data_root are skipped."""

    # === Connection Management ===
    def test_connection_cache_invalidation(self):
        """invalidate_cache() clears metadata cache."""

    def test_context_manager_closes_connection(self):
        """Connection closed on context manager exit."""
```

### 3. docs/CONCEPTS/crsp-data.md

Documentation explaining CRSP data concepts and usage.

---

## Implementation Strategy

### Step 1: Core Provider + Security (Day 1)
1. Create `CRSPLocalProvider` class with security validation
2. Implement path validation against `data_root`
3. Implement `_ensure_connection()` with DuckDB safety PRAGMAs
4. Implement `_get_manifest()` with DataNotFoundError
5. Write security and connection tests

### Step 2: Manifest-Aware Queries (Day 2)
1. Implement `_get_partition_paths_from_manifest()`
2. Implement `_execute_query()` with parameterized SQL
3. Implement `get_daily_prices()` with manifest version pinning
4. Write query and manifest consistency tests

### Step 3: Point-in-Time & Universe (Day 3)
1. Implement `_get_security_metadata()` for first/last dates
2. Implement `get_universe()` with survivorship options
3. Implement `ticker_to_permno()` with ambiguity handling
4. Implement `permno_to_ticker()` with date awareness
5. Write point-in-time and mapping tests

### Step 4: Polish & Documentation (Day 4)
1. Implement `adjust_prices` flag for negative prices
2. Add comprehensive logging
3. Create `docs/CONCEPTS/crsp-data.md`
4. Run full test suite
5. Code review and fixes

---

## Test Fixtures

```python
@pytest.fixture
def mock_crsp_data(tmp_path: Path) -> tuple[Path, ManifestManager]:
    """Create mock CRSP parquet files and manifest for testing.

    Creates data/wrds/crsp/daily/ structure with:
    - PERMNO 10001: AAPL, trades 2020-01-02 to present
    - PERMNO 10002: DELISTED, trades 2020-01-02 to 2021-12-15 (delisted)
    - PERMNO 10003: NEWIPO, trades 2022-01-03 to present (IPO in 2022)
    - PERMNO 10004: RENAMED, trades 2020-01-02 to present (ticker changed mid-2021)
    - PERMNO 10005: AMBIG, trades 2021-06-01 to 2021-12-31 (same ticker as 10004 overlap)
    """
    # Create directory structure per P4T1_TASK.md
    crsp_dir = tmp_path / "data" / "wrds" / "crsp" / "daily"
    crsp_dir.mkdir(parents=True)

    # Generate test data for 2020, 2021, 2022...
    # Include weekends/holidays to test gap handling
    # Include negative prices for bid/ask average
```

---

## Edge Cases to Handle

1. **Empty date range**: Query for dates with no trading data → empty DataFrame
2. **Future dates**: Query for dates beyond data → empty DataFrame
3. **Missing partitions**: Years not in manifest → skip gracefully
4. **Invalid PERMNO/ticker**: Non-existent identifiers → DataNotFoundError
5. **Ticker collisions**: Same ticker, different PERMNOs → AmbiguousTickerError
6. **Price signs**: CRSP negative prices → raw or abs() with flag
7. **Path traversal**: Manifest paths outside data_root → skip with warning
8. **Manifest changes**: Version changes during query → ManifestVersionChangedError

---

## Success Criteria

1. All 25+ test cases pass
2. mypy --strict passes with no errors
3. >85% code coverage
4. Gemini + Codex code review approved
5. Documentation complete
6. No security vulnerabilities (injection, traversal)

---

## Review Checklist

Before requesting code review:
- [ ] All tests pass locally
- [ ] mypy --strict passes
- [ ] ruff format/lint passes
- [ ] No security issues (path traversal, injection)
- [ ] Parameterized DuckDB queries (no string formatting)
- [ ] Manifest version pinning implemented
- [ ] Documentation complete
- [ ] Error handling comprehensive
- [ ] Logging added for debugging
